  ...
  evalResponse(
    ...
    const msg = body.message
      ? this.translate.instant(
          body.message,
          body.message_params ? body.message_params : undefined
        )
      : "";
    if (status && +status === EnumStaticStatusCode.ERROR) {
      if (
        responseManager &&
        responseManager.fnError &&
        responseManager.fnError.flag
      ) {
      ...
      if (
        ...
        responseManager.toMessage.skipError
      ) {
        return true;
      }
      const exception = body.exception
        ? this.translate.instant(body.exception)
        : "";
