...
export class TypologicalModel extends ApiModel {
	private _cod: string;
	...
	titleHtml(cssImg?: { cssClass; cssStyle }, cssText?: { cssClass; cssStyle }): string {
		const cssImgClass = cssImg && cssImg.cssClass ? cssImg.cssClass : 'img-fluid select-image-icon';
		const cssImgStyle = cssImg && cssImg.cssStyle ? cssImg.cssStyle : '';
		const cssTextClass = cssText && cssText.cssClass ? cssText.cssClass : '';
