	...
	describe('with SecurityContext.NONE', () => {
		...
		describe('parse', () => {
			...
			it('should throw when emoji is true but emoji-toolkit is not loaded', () => {
				...
				expect(() =>
					markdownService.parse('I :heart: ngx-markdown', { decodeHtml: false, emoji: true }),
			...
			it('should call joypixels when emoji is true', () => {
				...
				expect(markdownService.parse(mockRaw, { decodeHtml: false, emoji: true })).toEqual(
					marked.parse(mockEmojified),
				);
		...
		describe('render', () => {
			...
			it('should throw when clipboard is called but not loaded', () => {
				...
				expect(() => markdownService.render(container, { clipboard: true })).toThrowError(
					errorClipboardNotLoaded,
				);
			});
			...
			it('should throw when katex is called but not loaded', () => {
				...
				expect(() => markdownService.render(element, { katex: true })).toThrowError(
					errorKatexNotLoaded,
				);
			...
			it('should throw when mermaid is called but not loaded', () => {
				...
				expect(() => markdownService.render(container, { mermaid: true })).toThrowError(
					errorMermaidNotLoaded,
				);
