 ...
 */
@Injectable()
export class OAuthSocialService {
	...
	refreshAuthToken(providerId: string): Promise<void> {
		return new Promise((resolve, reject) => {
			if (!this.initialized) {
				reject(OAuthSocialService.ERR_NOT_INITIALIZED);
			} else if (providerId !== GoogleLoginProvider.PROVIDER_ID) {
				reject(OAuthSocialService.ERR_NOT_SUPPORTED_FOR_REFRESH_TOKEN);
			} else {
				...
				if (providerObject) {
					...
					reject(OAuthSocialService.ERR_LOGIN_PROVIDER_NOT_FOUND);
				}
			}
		});
	}
	...
	signOut(revoke: boolean = false): Promise<void> {
		return new Promise((resolve, reject) => {
			if (!this.initialized) {
				...
				reject(OAuthSocialService.ERR_NOT_LOGGED_IN);
			} else {
	...
	infos(providerId: string): Promise<void> {
		return new Promise((resolve, reject) => {
			if (!this.initialized) {
				reject(OAuthSocialService.ERR_NOT_INITIALIZED);
			} else {
